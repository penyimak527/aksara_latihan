<div class="card">
	<div class="card-header border-bottom border-dashed d-flex align-items-center justify-content-between">
		<div>
			<h4 class="header-title mb-1">Master Mata Pelajaran</h4>
			<small class="text-muted">Kelola data mata pelajaran yang dipakai untuk Bank Soal, Sesi Soal, dan Analisa
				Hasil Belajar.</small>
		</div>
	...
	<div class="card-body">
		<div class="row g-2 mb-3">
			<div class="col-md-8">
				<div class="input-group">
					<input type="text" class="form-control" id="cari" placeholder="Cari mata pelajaran ..."
			...
			<div class="col-md-4">
				<select class="form-control" id="filter_status" onchange="mata_pelajaran()">
		...
		<div id="data_mata_pelajaran"></div>
		<div
...
<div class="modal fade" id="edit" tabindex="-1" aria-hidden="true">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="form-edit">
					<input type="hidden" name="id_mata_pelajaran" id="id_mata_pelajaran">
					<div class="mb-2">
						<label class="form-label">Nama Mata Pelajaran</label>
						<input type="text" name="nama_mata_pelajaran" id="nama_mata_pelajaran" class="form-control">
					</div>
					<div class="mb-2">
						<label class="form-label">Keterangan</label>
...
<script>
	$(document).ready(function () {
		...
		$('#btn-simpan').click(function () {
			$.ajax({
				url: '<?= base_url('latihan_soal/master/mata_pelajaran/tambah'); ?>',
				type: 'POST',
				dataType: 'json',
		...
		$('#btn-update').click(function () {
			$.ajax({
				url: '<?= base_url('latihan_soal/master/mata_pelajaran/edit'); ?>',
				type: 'POST',
				dataType: 'json',
	...
	function mata_pelajaran() {
		...
		$.ajax({
			url: '<?= base_url('latihan_soal/master/mata_pelajaran/mata_pelajaran_result'); ?>',
			type: 'POST',
			dataType: 'json',
			data: {
				search,
				...
				if (!response_status || data.length == 0) {
					...
					data.forEach(function (item) {
						...
						table += `<div class="card-mapel">
							<div class="keterangan-mapel">
								<div class="keterangan-mapel-kiri">
									<h5 class="judul-mapel">${no++}. ${escapeHtml(item.nama_mata_pelajaran)}</h5>
									<p class="keterangan-jam-mapel" style="color:#343a40; margin-bottom:4px;">Status: ${badgeStatus(item.status_aktif)}</p>
									<p class="keterangan-jam-mapel" style="color:#343a40; margin-bottom:0;">Dibuat: ${escapeHtml(item.created_at || '-')}</p>
								</div>
	...
	function edit(detail) {
		...
		$('#id_mata_pelajaran').val(item.id);
		$('#nama_mata_pelajaran').val(item.nama_mata_pelajaran);
	...
	function ubah_status(id, status) {
		Swal.fire({ icon: 'question', title: 'Konfirmasi', text: 'Yakin ingin mengubah status data ini?', showCancelButton: true, confirmButtonText: 'Ya', cancelButtonText: 'Batal' }).then(function (result) {
			...
			$.ajax({
				url: '<?= base_url('latihan_soal/master/mata_pelajaran/ubah_status'); ?>',
				type: 'POST',
				dataType: 'json',
