	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="form-tambah">
					<div class="mb-2">
						<label for="simpleinput" class="form-label">Nama Beasiswa</label>
						<input type="text" name="nama_beasiswa" class="form-control" placeholder="Nama Beasiswa ...">
					</div>
					<div class="mb-2">
						<label for="simpleinput" class="form-label">Tipe</label>
					...
					<div class="mb-2">
						<label for="simpleinput" class="form-label">Nilai</label>
						<input type="text" name="nilai" class="form-control" onkeyup="FormatCurrency(this);"
							placeholder="Nilai ...">
					</div>
					<div class="mb-2">
...
<div class="modal fade" id="edit" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="form-edit">
					<input type="hidden" id="id_beasiswa" name="id_beasiswa" class="form-control">
					<div class="mb-2">
						<label for="simpleinput" class="form-label">Nama Beasiswa</label>
						<input type="text" id="nama_beasiswa" name="nama_beasiswa" class="form-control">
					</div>
					<div class="mb-2">
						<label for="simpleinput" class="form-label">Tipe</label>
					...
					<div class="mb-2">
						<label for="simpleinput" class="form-label">Nilai</label>
						<input type="text" id="nilai" name="nilai" class="form-control" onkeyup="FormatCurrency(this);">
					</div>
					<div class="mb-2">
						<label for="simpleinput" class="form-label">Keterangan</label>
...
<script>
	$(document).ready(function () {
		...
		$("#btn-simpan").click(function () {
			...
			$.ajax({
				url: '<?= base_url('admin/master/beasiswa/tambah'); ?>',
				type: 'POST',
				data: data,
		...
		$("#btn-update").click(function () {
			...
			var data = form.serialize();
			$.ajax({
				url: '<?= base_url('admin/master/beasiswa/edit'); ?>',
				type: 'POST',
				data: data,
				success: function (data) {
					$("#edit").modal('hide');
					...
					data.forEach(function (item) {
						...
						table += `<div class="card-mapel">
									<p class="keterangan-hari">
									  ...
									  <div class="keterangan-mapel">
										  <div class="keterangan-mapel-kiri">
											  <h5 class="judul-mapel" style="margin:0; margin-top: 8px;">${no++}. ${item.nama_beasiswa}</h5>
														<p style="margin: 0; padding: 0; font-size: 12px; margin-bottom: 8px;">
														<b>Tipe:</b> ${item.tipe} , <b>Nilai:</b> ${NumberToMoney(item.nilai)}
														<br>
	...
	function edit(detail) {
		...
		$('#id_beasiswa').val(item.id);
		$('#nama_beasiswa').val(item.nama_beasiswa);
		$('#form-edit select[name="tipe"]').html(
			`
			  ...
			  <option value="Harga Khusus" ${item.tipe == 'Harga Khusus' ? 'selected' : ''}>Harga Khusus</option>
			`
		)
		$('#nilai').val(NumberToMoney(item.nilai));
	...
	function hapus(id) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: 'POST',
					url: `<?= base_url(); ?>admin/master/beasiswa/hapus`,
					data: {
						id: id
					},
					dataType: 'json',
