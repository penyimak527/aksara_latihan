...
<div class="modal fade" id="tambah" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
          ...
          <div class="mb-2">
						<label for="simpleinput" class="form-label">Urutan Kelas</label>
						<input type="text" name="urutan_kelas" class="form-control" placeholder="Urutan Kelas ...">
					</div>
				</form>
			</div>
...
<div class="modal fade" id="edit" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
          ...
          <div class="mb-2">
						<label for="simpleinput" class="form-label">Urutan Kelas</label>
						<input type="text" id="urutan_kelas" name="urutan_kelas" class="form-control">
					</div>
				</form>
			</div>
			<div class=" modal-footer">
...
<script>
    ...
    jenjang();
		...
		$("#btn-simpan").click(function () {
			...
			$.ajax({
				url: '<?= base_url('admin/master/kelas/tambah'); ?>',
				type: 'POST',
				data: data,
		...
		$("#btn-update").click(function () {
			...
			var data = form.serialize();
			$.ajax({
				url: '<?= base_url('admin/master/kelas/edit'); ?>',
				type: 'POST',
				data: data,
				success: function (data) {
					$("#edit").modal('hide');
            ...
            if (item.status_aktif == '1') {
              status_aktif = 'Aktif'
						...
						table += `<div class="card-mapel">
                            ...
                            <p style="margin: 0; padding: 0; font-size: 12px; margin-bottom: 8px;"><b>Jenjang:</b> ${item.nama_jenjang} , <b>Urutan:</b> ${item.urutan_kelas}</p>
          								</div>
    ...
    $('#urutan_kelas').val(item.urutan_kelas);
	...
	function hapus(id) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: 'POST',
					url: `<?= base_url(); ?>admin/master/kelas/hapus`,
					data: {
						id: id
					},
					dataType: 'json',
