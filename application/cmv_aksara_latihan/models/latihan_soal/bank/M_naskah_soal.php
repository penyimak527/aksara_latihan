...
class M_naskah_soal extends CI_Model
{
    ...
    public function detail_naskah_pdf($id_naskah)
    {
        $id_naskah = (int) $id_naskah;
        if ($id_naskah <= 0) {
            return ['status' => false, 'message' => 'ID naskah soal tidak valid.'];
        }

        $naskah = $this->db
            ->select('a.*, b.nama_mata_pelajaran, c.nama_kategori_soal')
            ->from('soal_naskah a')
            ->join('mata_pelajaran b', 'a.id_mata_pelajaran = b.id', 'left')
            ->join('soal_kategori c', 'a.id_kategori_soal = c.id', 'left')
            ->where('a.id', $id_naskah)
            ->get()
            ->row_array();

        if (!$naskah) {
            return ['status' => false, 'message' => 'Naskah soal tidak ditemukan.'];
        }

        $soal = $this->db
            ->select('a.id, a.nomor_soal, a.id_materi, a.tipe_soal, a.bobot_nilai, a.pertanyaan, a.gambar_soal, a.status_aktif, b.nama_materi')
            ->from('soal a')
            ->join('materi b', 'a.id_materi = b.id', 'left')
            ->where('a.id_naskah_soal', $id_naskah)
            ->where('a.status_hapus IS NULL', null, false)
            ->where('a.status_aktif', '1')
            ->order_by('CAST(a.nomor_soal AS UNSIGNED)', 'ASC', false)
            ->get()
            ->result_array();

        foreach ($soal as $key => $row) {
            $jawaban = $this->db
                ->select('label_jawaban, isi_jawaban, urutan')
                ->from('soal_jawaban')
                ->where('id_soal', (int) $row['id'])
                ->order_by('urutan', 'ASC')
                ->get()
                ->result_array();

            $soal[$key]['tipe_soal_label'] = $this->tipe_soal_label($row['tipe_soal']);
            $soal[$key]['jawaban'] = $jawaban;
        }

        $naskah['jumlah_soal'] = count($soal);
