				...
				<div class="mb-3">
					<div class="input-group">
						<input type="text" class="form-control" id="cari" placeholder="Cari ..."
							aria-describedby="inputGroupPrepend" onkeyup="paket()">
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="form-tambah">
					<div class="mb-2">
						<label for="simpleinput" class="form-label">Nama Paket</label>
						<input type="text" name="nama_paket" class="form-control" placeholder="Nama Paket ...">
					</div>
          <div class="mb-2">
						<label for="simpleinput" class="form-label">Urutan</label>
...
<script>
	$(document).ready(function () {
		...
		$("#btn-simpan").click(function () {
			...
			$.ajax({
				url: '<?= base_url('admin/master/paket/tambah'); ?>',
				type: 'POST',
				data: data,
		...
		$("#btn-update").click(function () {
			...
			var data = form.serialize();
			$.ajax({
				url: '<?= base_url('admin/master/paket/edit'); ?>',
				type: 'POST',
				data: data,
				success: function (data) {
					$("#edit").modal('hide');
		...
		$('#dt-length-0').on('change', function () {
			...
			paging($('#data_paket .card-mapel'), jumlah);
		});
	})
              ...
              status_aktif = 'Aktif'
						...
						table += `<div class="card-mapel">
          						  <p class="keterangan-hari">
          							...
          							<div class="keterangan-mapel">
          								<div class="keterangan-mapel-kiri">
          									<h5 class="judul-mapel" style="margin:0; margin-top: 8px;">${item.urutan}. ${item.nama_paket}</h5>
          								</div>
				...
				$('#data_paket').html(table);
				let jumlah_awal = parseInt($('#dt-length-0').val());
				paging($('#data_paket .card-mapel'), jumlah_awal);
			}
		});
	}
	...
	function hapus(id) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: 'POST',
					url: `<?= base_url(); ?>admin/master/paket/hapus`,
					data: {
						id: id
					},
					dataType: 'json',
