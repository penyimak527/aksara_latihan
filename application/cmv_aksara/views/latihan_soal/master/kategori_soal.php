<div class="card">
	...
	<div class="card-body">
		<div class="row g-2 mb-3">
			...
			<div class="col-md-4">
				<select class="form-control" id="filter_status" onchange="kategori_soal()">
		...
		<div class="d-flex justify-content-between align-items-center mb-2">
			<h5 class="mb-0">Data Kategori Soal</h5>
			<small class="text-muted" id="total_data">0 data</small>
		</div>
		<div id="data_kategori_soal"></div>
		<div
...
<div class="modal fade" id="edit" tabindex="-1" aria-hidden="true">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title">Edit Kategori Soal</h4>
				<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
			</div>
			<div class="modal-body">
				<form id="form-edit">
					<input type="hidden" name="id_kategori_soal" id="id_kategori_soal">
					<div class="mb-2">
						<label class="form-label">Nama Kategori Soal</label>
						<input type="text" name="nama_kategori_soal" id="nama_kategori_soal" class="form-control">
					</div>
					<div class="mb-2">
						<label class="form-label">Keterangan</label>
...
<script>
	$(document).ready(function () {
		...
		$('#btn-simpan').click(function () {
			$.ajax({
				url: '<?= base_url('latihan_soal/master/kategori_soal/tambah'); ?>',
				type: 'POST',
				dataType: 'json',
		...
		$('#btn-update').click(function () {
			$.ajax({
				url: '<?= base_url('latihan_soal/master/kategori_soal/edit'); ?>',
				type: 'POST',
				dataType: 'json',
		...
		$('#dt-length-0').on('change', function () {
			...
			paging($('#data_kategori_soal .card-mapel'), jumlah);
		});
	});
	...
	function kategori_soal() {
		var search = $('#cari').val();
		var status = $('#filter_status').val();

		$.ajax({
			url: '<?= base_url('latihan_soal/master/kategori_soal/kategori_soal_result'); ?>',
			type: 'POST',
			dataType: 'json',
			data: {
				search,
			...
			success: function (res) {
				...
				if (!response_status || data.length == 0) {
					...
					data.forEach(function (item) {
						...
						table += `<div class="card-mapel">
							<div class="keterangan-mapel">
								<div class="keterangan-mapel-kiri">
									<h5 class="judul-mapel">${no++}. ${escapeHtml(item.nama_kategori_soal)}</h5>
									<p class="keterangan-jam-mapel" style="color:#343a40; margin-bottom:4px;">Keterangan: ${escapeHtml(item.keterangan || '-')}</p>
								...
								<div class="keterangan-mapel-kanan">
									<div class="d-flex justify-content-center gap-2">
										<button type="button" class="btn btn-outline-warning 1-50" onclick="edit('${detail}')"><i class="ri-edit-line"></i></button>
				...
				$('#data_kategori_soal').html(table);
				let jumlah_awal = parseInt($('#dt-length-0').val());
				paging($('#data_kategori_soal .card-mapel'), jumlah_awal);
			}
		});
	}
	function edit(detail) {
		let item = JSON.parse(decodeURIComponent(escape(atob(detail))));
		$('#id_kategori_soal').val(item.id);
		$('#nama_kategori_soal').val(item.nama_kategori_soal);
	...
	function ubah_status(id, status) {
		Swal.fire({ icon: 'question', title: 'Konfirmasi', text: 'Yakin ingin mengubah status data ini?', showCancelButton: true, confirmButtonText: 'Ya', cancelButtonText: 'Batal' }).then(function (result) {
			...
			$.ajax({
				url: '<?= base_url('latihan_soal/master/kategori_soal/ubah_status'); ?>',
				type: 'POST',
				dataType: 'json',
