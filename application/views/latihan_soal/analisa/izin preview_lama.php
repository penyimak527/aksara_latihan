...
<script>
				...
				html += `<div class="card-mapel">
					<div class="keterangan-mapel">
						<div class="keterangan-mapel-kiri">
							...
							<p class="mb-1"><b>Sesi</b> : ${escapeHtml(row.nama_sesi)}<br>
							<b>Jenis Pengerjaan</b> : ${escapeHtml(row.jenis_pengerjaan)}</p>
						</div>
						<div class="keterangan-mapel-kanan" style="min-width:220px;">
							<label class="form-label">Status Izin Preview</label>
							<select class="form-control mb-2" id="preview-${row.id}">
								<option value="0" ${checked0}>Belum Diizinkan</option>
								<option value="1" ${checked1}>Diizinkan</option>
							</select>
							<button type="button" class="btn btn-primary w-100" onclick="simpanIzin('${row.id}')">Simpan</button>
						</div>
					</div>
				</div>`;
			});
	...
	function simpanIzin(id) {
		$.ajax({
			...
			data: {
				id_pengerjaan: id,
				preview_diizinkan: $(`#preview-${id}`).val()
			},
			success: function (res) {
				if (res.result == 'true') {
