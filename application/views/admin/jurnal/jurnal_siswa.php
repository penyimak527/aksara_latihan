...
<div class="card">
	...
	<div class="card-body">
		<div class="row">
			<div class="col-md-3">
				<div class="mb-3">
					<div class="input-group">
						<select class="form-control" id="id_kelas"></select>
					</div>
				</div>
			</div>
			<div class="col-md-3">
			...
			<div class="col-md-3">
				<div class="mb-3">
					<button type="button" class="btn btn-sm btn-primary" onclick="jurnal_siswa()">
						<i class="ri-search-line"></i></button>
				</div>
			</div>
		</div>
		<div id="data_jurnal_siswa"></div>
		<div
					...
					<div class="row">
						<div class="col-md-12">
							...
							<div class="mb-2">
								...
								<select name="id_kelas_setting" class="form-control" onchange="pilih_kelas('tambah');">
								</select>
							</div>
...
<div class="modal fade" id="edit" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-xlg">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="form-edit">
					<input type="hidden" id="id_jurnal_siswa" name="id_jurnal_siswa" class="form-control">
					<div class="mb-2">
						<label for="simpleinput" class="form-label">Tanggal</label>
...
<script>
	$(document).ready(function () {
		...
		$("#btn-simpan").click(function () {
			...
			$.ajax({
				url: '<?= base_url('admin/jurnal/jurnal_siswa/tambah'); ?>',
				type: 'POST',
				data: data,
		...
		$("#btn-update").click(function () {
			$('#btn-update').prop('disabled', true).html(`Loading...`);
			var form = $("#form-edit");
			var data = form.serialize();
			$.ajax({
				url: '<?= base_url('admin/jurnal/jurnal_siswa/edit'); ?>',
				type: 'POST',
				data: data,
				success: function (data) {
					$("#edit").modal('hide');
					...
					$('#btn-update').prop('disabled', false).html(`Simpan`);
				}
			})
		})
		...
		$('#dt-length-0').on('change', function () {
			...
			paging($('#data_jurnal_siswa .card-mapel'), jumlah);
		});
	})
	function tampiltambah_modal() {
		$.ajax({
			url: '<?= base_url('admin/jurnal/jurnal_siswa/cek_session'); ?>',
			type: 'POST',
			dataType: 'JSON',
			success: function (data) {
				const status_log = data.status;
				if (status_log == 'logout') {
	...
	function jurnal_siswa() {
		var id_kelas = $("#id_kelas").val();
		var tanggal = $("#flatpicker").val();

		$.ajax({
			url: '<?= base_url('admin/jurnal/jurnal_siswa/jurnal_siswa_result'); ?>',
			type: 'POST',
			data: {
				id_kelas,
				tanggal
			...
			success: function (data) {
				...
				$('#data_jurnal_siswa').html(table);
				let jumlah_awal = parseInt($('#dt-length-0').val());
				paging($('#data_jurnal_siswa .card-mapel'), jumlah_awal);
			}
		});
	}
	...
	function edit(detail) {
		...
		$('#form-edit input[name="id_pegawai"]').val(item.id_pegawai);
	...
	function hapus(id) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: 'POST',
					url: `<?= base_url(); ?>admin/jurnal/jurnal_siswa/hapus`,
					data: {
						id: id
					},
					dataType: 'json',
	...
	function kelas(id_kelas = null) {
		let id_pegawai = <?php echo $this->session->userdata('admin')['id_pegawai']; ?>

		$.ajax({
			url: '<?= base_url('admin/jurnal/jurnal_siswa/kelas_result'); ?>',
			data: { id_pegawai },
			type: 'POST',
			dataType: 'JSON',
			success: function (data) {
				...
				if (id_kelas == null) {
					$('#form-tambah select[name="id_kelas_setting"]').html(option);
					$('#id_kelas').html(option);
				} else {
					$('#form-edit select[name="id_kelas_setting"]').html(option);
				}
			}
		});
	}
	...
	function pilih_kelas(status_form) {
		...
		if (status_form == 'detail' || status_form == 'edit') {
			...
			$.ajax({
				url: '<?= base_url('admin/jurnal/jurnal_siswa/detail_siswa_result'); ?>',
			...
			if (status_form == 'tambah') {
				...
				id_jenjang = $('#form-tambah select[name="id_kelas_setting"]').find(':selected').data('jenjang');
				nama_jenjang = $('#form-tambah select[name="id_kelas_setting"]').find(':selected').data('namajenjang');
				id_kelas = $('#form-tambah select[name="id_kelas_setting"]').find(':selected').data('kelas');
				nama_kelas = $('#form-tambah select[name="id_kelas_setting"]').find(':selected').data('namakelas');
			...
			$.ajax({
				url: '<?= base_url('admin/jurnal/jurnal_siswa/siswa_result'); ?>',
